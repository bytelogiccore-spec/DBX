"""
DBX vs SQLite - Performance Comparison Benchmark (with Transaction)

Compares DBX Python bindings with SQLite (sqlite3 module).
Uses transaction context manager for batching.
"""
...
def benchmark_dbx_with_transaction(n=10000):
    """Benchmark DBX operations with transaction"""
...
def main():
    print("=" * 60)
    print("DBX vs SQLite - Performance Comparison (with Transaction)")
    print("=" * 60)
    
    n = 10000
    print(f"\nRunning benchmarks with {n:,} operations...\n")
    
    # Benchmark DBX with transaction
    print("Benchmarking DBX (with transaction)...")
    dbx_insert, dbx_get, dbx_delete = benchmark_dbx_with_transaction(n)
    print_results("DBX (In-Memory, Transaction)", dbx_insert, dbx_get, dbx_delete, n)
