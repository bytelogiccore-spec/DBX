...
fn bench_two_level_cache_large_data(c: &mut Criterion) {
    ...
    let cache_l1 = TwoLevelCache::new(
        10 * 1024 * 1024,
        PathBuf::from("target/bench_cache_l1_large"),
    );
...
fn bench_parallel_sql_parser_complex(c: &mut Criterion) {
    ...
    group.bench_function("single_subquery", |b| {
        b.iter(|| parser.parse(black_box(subquery)).unwrap())
    });
    ...
    group.bench_function("batch_parse_1000", |b| {
        b.iter(|| parser.parse_batch(black_box(&sqls_1000_refs)).unwrap())
    });
    ...
    group.bench_function("batch_parse_10000", |b| {
        b.iter(|| parser.parse_batch(black_box(&sqls_10000_refs)).unwrap())
    });
