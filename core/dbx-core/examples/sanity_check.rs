...
fn main() -> dbx_core::DbxResult<()> {
    ...
    if !results.is_empty() && results[0].num_rows() > 0 {
        let val_arr = results[0].column(0).as_any().downcast_ref::<StringArray>().unwrap();
        let val = val_arr.value(0);
        println!("✅ Result: {}", val);
        if val == "data_5000" {
            println!("🌟 SUCCESS: Data matches exactly!");
        } else {
            println!("❌ ERROR: Data mismatch! Expected 'data_5000', got '{}'", val);
        }
    } else {
    ...
    if !results.is_empty() {
        ...
        let val_arr = results[0].column(1).as_any().downcast_ref::<StringArray>().unwrap();
