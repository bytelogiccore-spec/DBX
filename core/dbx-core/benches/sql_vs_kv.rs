...
fn setup_db_with_data(num_rows: usize) -> Database {
    ...
    // CREATE TABLE
    db.execute_sql("CREATE TABLE users (id INT, name TEXT, age INT, score FLOAT)").unwrap();
    
    // INSERT 데이터
    for i in 0..num_rows {
        let sql = format!(
            "INSERT INTO users VALUES ({}, 'User{}', {}, {})",
            i, i, 20 + (i % 50), 50.0 + (i as f64 * 0.1)
        );
...
fn setup_db_with_index(num_rows: usize) -> Database {
    ...
    // CREATE INDEX
    db.execute_sql("CREATE INDEX idx_age ON users(age)").unwrap();
...
fn bench_update_sql_single(c: &mut Criterion) {
    c.bench_function("update_sql_single", |b| {
        ...
        b.iter(|| {
            let sql = format!("UPDATE users SET score = {} WHERE id = {}", 
                            100.0 + counter as f64, counter % 1000);
            db.execute_sql(&sql).unwrap();
            counter += 1;
        });
    });
...
fn bench_update_kv_single(c: &mut Criterion) {
    c.bench_function("update_kv_single", |b| {
        ...
        b.iter(|| {
            ...
            let value = format!("{{\"id\":{},\"name\":\"User{}\",\"age\":{},\"score\":{}}}",
                              counter % 1000, counter % 1000, 25, 100.0 + counter as f64);
            db.put(black_box(key.as_bytes()), black_box(value.as_bytes())).unwrap();
            counter += 1;
        });
    });
}
...
fn bench_update_sql_with_index(c: &mut Criterion) {
    c.bench_function("update_sql_with_index", |b| {
        ...
        b.iter(|| {
            let sql = format!("UPDATE users SET score = {} WHERE age = {}", 
                            150.0 + counter as f64, 20 + (counter % 50));
            db.execute_sql(&sql).unwrap();
            counter += 1;
        });
    });
...
fn bench_update_kv_with_index(c: &mut Criterion) {
    c.bench_function("update_kv_with_index", |b| {
        ...
        b.iter(|| {
            ...
            let key = format!("user:{}", counter % 1000);
            let value = format!("{{\"id\":{},\"name\":\"User{}\",\"age\":{},\"score\":{}}}",
                              counter % 1000, counter % 1000, 25, 150.0 + counter as f64);
            db.put(key.as_bytes(), value.as_bytes()).unwrap();
            counter += 1;
        });
    });
