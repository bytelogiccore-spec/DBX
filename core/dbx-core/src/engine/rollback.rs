...
impl Checkpoint {
    ...
    pub fn get_state<T: for<'de> Deserialize<'de>>(&self, key: &str) -> DbxResult<T> {
        let json_value = self.state.get(key).ok_or_else(|| {
            DbxError::Serialization(format!("State key '{}' not found", key))
        })?;
...
impl RollbackManager {
    ...
    pub fn create_checkpoint(&self, id: String, description: String) -> DbxResult<Checkpoint> {
        ...
        // 메모리에 저장
        self.checkpoints.write().unwrap().insert(id.clone(), checkpoint.clone());
    ...
    pub fn rollback_to_checkpoint(&self, id: &str) -> DbxResult<Checkpoint> {
        ...
        // 메모리에 저장
        self.checkpoints.write().unwrap().insert(id.to_string(), checkpoint.clone());
...
mod tests {
    ...
    fn test_rollback_to_checkpoint() {
        ...
        // 체크포인트 생성
        let mut checkpoint = manager.create_checkpoint(
            "test_cp_2".to_string(),
            "Rollback test".to_string(),
        ).unwrap();
        
        // 상태 데이터 추가
        checkpoint.add_state("key1".to_string(), &"value1".to_string()).unwrap();
        checkpoint.add_state("key2".to_string(), &42).unwrap();
        
        // 저장
        manager.checkpoints.write().unwrap().insert("test_cp_2".to_string(), checkpoint.clone());
    ...
    fn test_auto_rollback_on_regression() {
        ...
        manager.create_checkpoint(
            "test_cp_3".to_string(),
            "Auto-rollback test".to_string(),
        ).unwrap();
        
        // 자동 롤백 트리거
        manager.trigger_auto_rollback("Performance regression detected").unwrap();
