...
impl FeatureFlags {
    ...
    pub fn is_enabled(&self, feature: Feature) -> bool {
        self.flags.read().unwrap().get(&feature).copied().unwrap_or(false)
    }
        ...
        if let Some(path) = &self.persistence_path {
            if path.exists() {
                ...
                for (key, value) in loaded {
                    if let Some(feature) = Feature::from_str(&key) {
...
mod tests {
    ...
    fn test_feature_flag_persistence() {
        ...
        let flags = FeatureFlags::new()
            .with_persistence(temp_path.clone());
    ...
    fn test_feature_all_and_reset() {
        ...
        let all = flags.all();
        assert_eq!(all.len(), 2);
        assert!(all.get(&Feature::BinarySerialization).copied().unwrap_or(false));
    ...
    fn test_feature_from_str() {
        assert_eq!(
            Feature::from_str("binary_serialization"),
            Some(Feature::BinarySerialization)
        );
        assert_eq!(
            Feature::from_str("multi_threading"),
            Some(Feature::MultiThreading)
        );
        assert_eq!(Feature::from_str("invalid"), None);
    }
}
