...
pub struct DistributedChannels {
    ...
    pub shuffles: HashMap<usize, Vec<(std::net::SocketAddr, mpsc::Receiver<DbxResult<Option<Vec<u8>>>>)>>,
}
...
impl LocalExecutor {
    ...
    fn build_operator_internal(
        ...
        match plan {
            ...
            PhysicalPlan::GridExchange { exchange_id, schema_hint } => {
                if let Some(ch) = channels.as_mut() {
                    ...
                    Err(DbxError::SqlExecution {
                        message: "GridExchange encountered but no DistributedChannels provided".to_string(),
                        context: "LocalExecutor::build_operator_internal".to_string(),
                    })
                }
            }
            
            PhysicalPlan::ShuffleWriter { input, hash_params, target_nodes, exchange_id, salting } => {
                if let Some(ch) = channels.as_mut() {
                    ...
                    Err(DbxError::SqlExecution {
                        message: "ShuffleWriter encountered but no DistributedChannels provided".to_string(),
                        context: "LocalExecutor::build_operator_internal".to_string(),
                    })
                }
            }
