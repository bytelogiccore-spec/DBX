...
impl QuicChannel {
    ...
    pub async fn new(
        ...
        let tls_builder = s2n_quic::provider::tls::default::Server::builder()
            .with_certificate(cert_pem, key_pem).map_err(|e| crate::error::DbxError::Network(e.to_string()))?;
        let tls = tls_builder.build().map_err(|e| crate::error::DbxError::Network(e.to_string()))?;

        let mut server = s2n_quic::Server::builder()
            .with_tls(tls).map_err(|e| crate::error::DbxError::Network(e.to_string()))?
            .with_io(local_addr).map_err(|e| crate::error::DbxError::Network(e.to_string()))?
            .start().map_err(|e| crate::error::DbxError::Network(e.to_string()))?;
            
        // 인증 기관 체크 무시 (테스트 및 동일 그리드 허용) - 실무에선 인증서 검증 필수
        let client_tls_builder = s2n_quic::provider::tls::default::Client::builder()
            .with_certificate(cert_pem).map_err(|e| crate::error::DbxError::Network(e.to_string()))?;
        let client_tls = client_tls_builder.build().map_err(|e| crate::error::DbxError::Network(e.to_string()))?;

        let client = s2n_quic::Client::builder()
            .with_tls(client_tls).map_err(|e| crate::error::DbxError::Network(e.to_string()))?
            .with_io("0.0.0.0:0").map_err(|e| crate::error::DbxError::Network(e.to_string()))?
            .start().map_err(|e| crate::error::DbxError::Network(e.to_string()))?;
                        ...
                        tokio::spawn(async move {
                            ...
                            if stream.read_exact(&mut len_buf).await.is_ok() {
                                ...
                                if stream.read_exact(&mut data_buf).await.is_ok() {
                                    if let Ok(msg) = crate::grid::protocol::GridMessage::deserialize(&data_buf) {
                                        let _ = tx2.send(GridMessageWrapper {
                                            msg,
                                            stream: Some(stream),
                                        }).await;
                                    }
    ...
    pub async fn send_message(&self, peer_addr: SocketAddr, msg: crate::grid::protocol::GridMessage) -> DbxResult<()> {
        info!("Sending GridMessage to {}", peer_addr);
        
        // Connect
        let connect_config = s2n_quic::client::Connect::new(peer_addr).with_server_name("localhost");
    ...
    pub async fn send_request_and_wait(&self, peer_addr: SocketAddr, msg: crate::grid::protocol::GridMessage) -> DbxResult<crate::grid::protocol::GridMessage> {
        let connect_config = s2n_quic::client::Connect::new(peer_addr).with_server_name("localhost");
        let mut connection = match self.client.connect(connect_config).await {
            Ok(c) => c,
            Err(e) => return Err(crate::error::DbxError::Network(e.to_string())),
        ...
        use tokio::io::AsyncReadExt;
        let mut len_buf = [0u8; 4];
        stream.read_exact(&mut len_buf).await.map_err(|e| crate::error::DbxError::Network(e.to_string()))?;
        
        let reply_len = u32::from_be_bytes(len_buf) as usize;
        let mut reply_buf = vec![0u8; reply_len];
        
        stream.read_exact(&mut reply_buf).await.map_err(|e| crate::error::DbxError::Network(e.to_string()))?;
        
        crate::grid::protocol::GridMessage::deserialize(&reply_buf)
    }

    /// 정적 메서드로 스트림에 메시지 응답 전송
    pub async fn send_response(stream: &mut s2n_quic::stream::BidirectionalStream, msg: crate::grid::protocol::GridMessage) -> DbxResult<()> {
        let bytes = msg.serialize()?;
        let len = (bytes.len() as u32).to_be_bytes();
        
        use tokio::io::AsyncWriteExt;
        stream.write_all(&len).await.map_err(|e| crate::error::DbxError::Network(e.to_string()))?;
        stream.write_all(&bytes).await.map_err(|e| crate::error::DbxError::Network(e.to_string()))?;
        
        // 데이터 전송 완료 후 플러시 및 스트림 정상 종료(FIN) 대기
        stream.flush().await.map_err(|e| crate::error::DbxError::Network(e.to_string()))?;
        stream.shutdown().await.map_err(|e| crate::error::DbxError::Network(e.to_string()))?;
...
impl GridMessageWrapper {
    ...
    pub async fn send_reply(&mut self, reply: crate::grid::protocol::GridMessage) -> DbxResult<()> {
        if let Some(stream) = &mut self.stream {
            ...
            Err(crate::error::DbxError::Network("No stream available for reply".into()))
        }
    }
}
...
mod tests {
    ...
    async fn test_quic_channel_send_and_receive() {
        ...
        let channel1 = QuicChannel::new(node1_addr, &cert_pem, &key_pem, tx1).await.unwrap();
        ...
        let (tx1, mut rx1) = mpsc::channel(100);
        let channel1 = QuicChannel::new(node1_fixed_addr, &cert_pem, &key_pem, tx1).await.unwrap();

        let node2_fixed_addr: SocketAddr = "127.0.0.1:15683".parse().unwrap();
        let (tx2, _rx2) = mpsc::channel(100);
        let channel2 = QuicChannel::new(node2_fixed_addr, &cert_pem, &key_pem, tx2).await.unwrap();
        ...
        channel2.send_message(node1_fixed_addr, test_msg.clone()).await.unwrap();

        // node1 receives msg
        if let Some(wrapper) = rx1.recv().await {
            match wrapper.msg {
                crate::grid::protocol::GridMessage::Storage(StorageMessage::FetchShard { key, shard_id }) => {
