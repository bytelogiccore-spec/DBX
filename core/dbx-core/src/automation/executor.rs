...
impl ExecutionEngine {
    ...
    pub fn register(&self, callable: Arc<dyn Callable>) -> DbxResult<()> {
        ...
        let mut callables = self.callables.write()
            .map_err(|_| DbxError::LockPoisoned)?;
    ...
    pub fn execute(
        ...
        // Callable 조회
        let callables = self.callables.read()
            .map_err(|_| DbxError::LockPoisoned)?;
...
impl ExecutionMetrics {
    ...
    pub fn record(&mut self, name: &str, duration: Duration, success: bool) {
        ...
        *self.call_counts.entry(name.to_string()).or_insert(0) += 1;
        ...
        // 실행 시간
        *self.total_durations.entry(name.to_string()).or_insert(Duration::ZERO) += duration;
...
mod tests {
    ...
    fn test_register_and_execute() {
        ...
        // 실행
        let ctx = ExecutionContext::new(Arc::new(crate::engine::Database::open_in_memory().unwrap()));
        let result = engine.execute("test_func", &ctx, &[Value::Int(42)]).unwrap();
