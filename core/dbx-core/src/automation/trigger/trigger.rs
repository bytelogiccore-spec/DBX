...
pub enum TriggerAction {
    ...
    /// 커스텀 액션
    Custom(Box<dyn Fn(&ExecutionContext, &TriggerEvent) -> DbxResult<()> + Send + Sync>),
}
...
impl Callable for Trigger {
    fn call(&self, ctx: &ExecutionContext, _args: &[Value]) -> DbxResult<Value> {
...
mod tests {
    ...
    fn test_trigger_condition() {
        ...
        let trigger = Trigger::new(
            ...
            TriggerEventType::AfterInsert,
            "users",
            TriggerCondition::Custom(Box::new(|event| {
                event.data.contains_key("id")
            })),
            TriggerAction::Custom(Box::new(|_ctx, _event| Ok(()))),
        );
