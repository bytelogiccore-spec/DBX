...
fn test_grace_join_spill() {
    ...
    let right_batches = vec![
        create_batch(Arc::clone(&schema), 500, 1500),
    ];
...
fn test_recursive_partitioning() {
    ...
    let left_ids = Arc::new(Int32Array::from((0..100).map(|i| i * 32).collect::<Vec<_>>()));
    ...
    let right_ids = Arc::new(Int32Array::from((0..50).map(|i| i * 32).collect::<Vec<_>>()));
    let right_vals = Arc::new(Int32Array::from(vec![2; 50]));
    let right_batch = RecordBatch::try_new(Arc::clone(&schema), vec![right_ids, right_vals]).unwrap();
