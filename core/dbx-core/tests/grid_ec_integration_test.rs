...
async fn test_grid_ec_network_integration() -> DbxResult<()> {
    ...
    let dist_store1 = Arc::new(DistributedErasureCodingStore::new(local_store1, Arc::clone(&router), Some(Arc::clone(&quic1))));
    ...
    let dist_store2 = Arc::new(DistributedErasureCodingStore::new(local_store2, Arc::clone(&router), Some(Arc::clone(&quic2))));
    ...
    let recovered = dist_store2.retrieve(key).await?.expect("Data should be recovered");
