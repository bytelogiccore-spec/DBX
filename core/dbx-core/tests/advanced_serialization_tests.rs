...
fn test_compression_zstd() -> DbxResult<()> {
    ...
    println!("Compression ratio: {:.2}%", (compressed.len() as f64 / original.len() as f64) * 100.0);
    
    assert!(compressed.len() < original.len(), "Compressed data should be smaller");
