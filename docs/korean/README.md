...
**DBX**는 현대적인 HTAP(Hybrid Transactional/Analytical Processing) 워크로드를 위해 설계된 **4계층 하이브리드 스토리지(4-Tier Hybrid Storage)** 아키텍처 기반의 차세대 임베디드 데이터베이스입니다.
...
- **🚀 4계층 하이브리드 스토리지** — OLTP와 OLAP 워크로드 모두에 최적화
...
└─────────────────┬───────────────────────┘
                  │
┌─────────────────▼───────────────────────┐
│  Tier 3: WOS (BTreeMap/sled)            │  ← MVCC 스냅샷 격리
└─────────────────┬───────────────────────┘
                  │ Compaction
┌─────────────────▼───────────────────────┐
│  Tier 4: ROS (Parquet)                  │  ← 컬럼형 압축 저장
└─────────────────────────────────────────┘
...
### 핵심 기능 ✅
- ✅ **4계층 하이브리드 스토리지** — Delta → Cache → WOS → ROS
