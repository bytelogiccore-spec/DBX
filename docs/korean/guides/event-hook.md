...
데이터 변경 이벤트에 Rust 클로저를 등록하여 자동으로 실행할 수 있습니다.
{: .fs-6 .fw-300 }
...
EventHook은 Rust 클로저 기반으로 특정 테이블에서 INSERT, UPDATE, DELETE가 발생할 때 자동으로 실행되는 콜백 함수입니다.

> **참고**: SQL 표준 Trigger는 [Trigger 가이드](triggers)를 참조하세요.
...
- [SQL Trigger 가이드](triggers) — SQL 표준 Trigger 사용법
